...
if (isset($_GET['module']) and $_GET['module'] == 'search') {
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        $count = $db->query("SELECT COUNT(*) FROM `attributes` WHERE `client_id` = '" . $chief['id'] . "'")->fetch_row();
        if ($count[0] == 0) {
            ...
            if (count($search_values) != 0) {
                ...
                $count = $db->query("SELECT COUNT(*) FROM `attributes` WHERE `client_id` = '" . $chief['id'] . "' AND " . $chunk . "")->fetch_row();
                if ($count[0] == 0) {
                    $pagination = array('countPages' => 1, 'currentPage' => 1, 'totalRows' => 0, 'maxRows' => $items_on_page);
                    echo json_encode(array('rows' => null, 'pagination' => $pagination));
                    exit;
                    ...
                    $items = $db->query("SELECT `id_item`, `name`, `status`, `category_id`, `sort` FROM `attributes` WHERE `client_id` = '" . $chief['id'] . "' AND " . $chunk . "  ORDER by `sort` LIMIT $start, $items_on_page");
                    while ($attribute = $items->fetch_assoc()) {
                        $category = $db->query("SELECT `name` FROM `attribute_categories` WHERE `id_item` = '" . $attribute['category_id'] . "' AND `client_id` = '" . $chief['id'] . "'")->fetch_assoc();
                        $rows[] = array(
                            'id_item' => $attribute['id_item'],
                            'name' => protection($attribute['name'], 'display'),
                ...
                $items = $db->query("SELECT `id_item`, `name`, `status`, `category_id`, `sort` FROM `attributes` WHERE `client_id` = '" . $chief['id'] . "' ORDER by `sort` LIMIT $start, $items_on_page");
                while ($attribute = $items->fetch_assoc()) {
                    $category = $db->query("SELECT `name` FROM `attribute_categories` WHERE `id_item` = '" . $attribute['category_id'] . "' AND `client_id` = '" . $chief['id'] . "'")->fetch_assoc();
                    $rows[] = array(
                        'id_item' => $attribute['id_item'],
...
if (isset($_GET['show']) and $_GET['show'] == 'true') {
    ...
    if ($count[0] > 0) {
        ...
        $attributes = $db->query("SELECT `id_item`, `name`, `status`, `category_id`, `sort` FROM `attributes` WHERE `client_id` = '" . $chief['id'] . "' ORDER by `sort` LIMIT $start, $items_on_page");
