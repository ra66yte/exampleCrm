    ...
    if ($count[0] == 0) {
        ...
        if (count($search_values) != 0) {
            ...
            for ($i = 1; $i < count($search_values); $i++) {
                $search_keys[$i] != ''$chunk .= " AND `" . protection($search_keys[$i], 'base') . "` LIKE '%" . protection($search_values[$i], 'base') . "%'";
            }
            if ($type == "all") {
                $count = $db->query("SELECT COUNT(*) FROM `clients` WHERE `client_id` = '" . $chief['id'] . "' AND " . $chunk . "")->fetch_row();
                if ($count[0] == 0) {
                    $pagination = array('countPages' => 1, 'currentPage' => 1, 'totalRows' => 0, 'maxRows' => $items_on_page);
                    echo json_encode(array('rows' => null, 'pagination' => $pagination));
                    exit;
                    ...
                    $items = $db->query("SELECT `id_item`, `name`, `phone`, `email`, `comment`, `site`, `ip`, `group_id`, `country`, `date_added` FROM `clients` WHERE `client_id` = '" . $chief['id'] . "' AND " . $chunk . "  ORDER by `id` DESC LIMIT $start, $items_on_page");
                    while ($client = $items->fetch_assoc()) {
                ...
                // Ищем в категории $type
                $count = $db->query("SELECT COUNT(*) FROM `clients` WHERE `group_id` = '" . $type . "' AND `client_id` = '" . $chief['id'] . "' AND " . $chunk . "")->fetch_row();
                if ($count[0] == 0) {
                    $pagination = array('countPages' => 1, 'currentPage' => 1, 'totalRows' => 0, 'maxRows' => $items_on_page);
                    echo json_encode(array('rows' => null, 'pagination' => $pagination));
                    exit;
                    ...
                    $items = $db->query("SELECT `id_item`, `name`, `phone`, `email`, `comment`, `site`, `ip`, `group_id`, `country`, `date_added` FROM `clients` WHERE `group_id` = '" . $type . "' AND `client_id` = '" . $chief['id'] . "' AND " . $chunk . "  ORDER by `id` DESC LIMIT $start, $items_on_page");
                    while ($client = $items->fetch_assoc()) {
                        $client_group = $db->query("SELECT `name` FROM `groups_of_clients` WHERE `id_item` = '" . $client['group_id'] . "' AND `client_id` = '" . $chief['id'] . "'")->fetch_assoc();
                        if (!isset($client_group)) $client_group['name'] = 'Без группы';
                        $client_country = $db->query("SELECT `name`, `code` FROM `countries` WHERE `id` = '" . $client['country'] . "'")->fetch_assoc();
            ...
            if ($type == "all") {
                ...
                if ($result = $db->query("SELECT COUNT(*) FROM `clients` WHERE `group_id` = '" . $type . "' AND `client_id` = '" . $chief['id'] . "'")->fetch_row() and $result[0] == 0) { // если такой категории клиентов нет или в нем нет клиентов
                    $pagination = array('countPages' => 1, 'currentPage' => 1, 'totalRows' => 0, 'maxRows' => $items_on_page);
                ...
                $i = 1;
                while ($client = $clients->fetch_assoc()) {
                    $client_group = $db->query("SELECT `name` FROM `groups_of_clients` WHERE `id` = '" . $client['group_id'] . "' AND `client_id` = '" . $chief['id'] . "'")->fetch_assoc();
                    if (!isset($client_group)) $client_group['name'] = 'Без группы';
                    $client_country = $db->query("SELECT `name`, `code` FROM `countries` WHERE `id` = '" . $client['country'] . "'")->fetch_assoc();
                    if (!isset($client_country)) {
                        $client_country = array('code' => '', 'name' => '');
