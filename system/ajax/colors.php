...
if (isset($_GET['module']) and $_GET['module'] == 'search') {
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $count = $db->query("SELECT COUNT(*) FROM `colors` WHERE `client_id` = '" . $chief['id'] . "'")->fetch_row();
        if ($count[0] == 0) {
            ...
            if (count($search_values) != 0) {
                ...
                $count = $db->query("SELECT COUNT(*) FROM `colors` WHERE `client_id` = '" . $chief['id'] . "' AND " . $chunk . "")->fetch_row();
                ...
                if ($count[0] == 0) {
                    ...
                    $items = $db->query("SELECT `id_item`, `name`, `status`, `sort` FROM `colors` WHERE `client_id` = '" . $chief['id'] . "' AND " . $chunk . " ORDER by `sort` LIMIT $start, $items_on_page");
                    while ($color = $items->fetch_assoc()) {
                        $rows[] = array(
                            'id_item' => $color['id_item'],
                            'name' => protection($color['name'], 'display'),
                            'status' => $color['status'],
                            'sort' => $color['sort']
                        );
                    }
                    ...
                    $items = $db->query("SELECT `id_item`, `name`, `status`, `sort` FROM `colors` WHERE `client_id` = '" . $chief['id'] . "' ORDER by `id` LIMIT $start, $items_on_page");
                    while ($color = $items->fetch_assoc()) {
                        $rows[] = array(
                            'id_item' => $color['id_item'],
                            'name' => protection($color['name'], 'display'),
...
if (isset($_GET['show']) and $_GET['show'] == 'true') {
    ...
    if ($count[0] > 0) {
        ...
        $colors = $db->query("SELECT `id_item`, `name`, `status`, `sort` FROM `colors` WHERE `client_id` = '" . $chief['id'] . "' ORDER by `sort` LIMIT $start, $items_on_page");
