...
if (isset($_GET['module']) and $_GET['module'] == 'search') {
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $count = $db->query("SELECT COUNT(*) FROM `currencies` WHERE `client_id` = '" . $chief['id'] . "'")->fetch_row();
        if ($count[0] == 0) {
            ...
            if (count($search_values) != 0) {
                ...
                $count = $db->query("SELECT COUNT(*) FROM `currencies` WHERE `client_id` = '" . $chief['id'] . "' AND " . $chunk . "")->fetch_row();
                ...
                if ($count[0] == 0) {
                    ...
                    $items = $db->query("SELECT `id_item`, `name`, `symbol`, `sort` FROM `currencies` WHERE `client_id` = '" . $chief['id'] . "' AND " . $chunk . " ORDER by `id` LIMIT $start, $items_on_page");
                    while ($currency = $items->fetch_assoc()) {
                        $rows[] = array(
                            'id_item' => $currency['id_item'],
                            'name' => protection($currency['name'], 'display'),
                            'symbol' => protection($currency['symbol'], 'display'),
                            'sort' => $currency['sort']
                        );
                    }
                }
                    ...
                    $items = $db->query("SELECT `id_item`, `name`, `symbol`, `sort` FROM `currencies` WHERE `client_id` = '" . $chief['id'] . "' ORDER by `id` LIMIT $start, $items_on_page");
                    while ($currency = $items->fetch_assoc()) {
                        $rows[] = array(
                            'id_item' => $currency['id_item'],
                            'name' => protection($currency['name'], 'display'),
...
if (isset($_GET['show']) and $_GET['show'] == 'true') {
    ...
    if ($count[0] > 0) {
        ...
        $currentPage = page($countPages);
        $start = ($currentPage * $items_on_page) - $items_on_page;
        $currencies = $db->query("SELECT  `id_item`, `name`, `symbol`, `sort` FROM `currencies` WHERE `client_id` = '" . $chief['id'] . "' ORDER by `id` ASC LIMIT $start, $items_on_page");
