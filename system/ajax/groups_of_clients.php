...
if (isset($_GET['module']) and $_GET['module'] == 'search') {
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $count = $db->query("SELECT COUNT(*) FROM `groups_of_clients` WHERE `client_id` = '" . $chief['id'] . "'")->fetch_row();
        if ($count[0] == 0) {
            ...
            if (count($search_values) != 0) {
                ...
                $count = $db->query("SELECT COUNT(*) FROM `groups_of_clients` WHERE `client_id` = '" . $chief['id'] . "' AND " . $chunk . "")->fetch_row();
                if ($count[0] == 0) {
                    $pagination = array('countPages' => 1, 'currentPage' => 1, 'totalRows' => 0, 'maxRows' => $items_on_page);
                    echo json_encode(array('rows' => null, 'pagination' => $pagination));
                    exit;
                    ...
                    $items = $db->query("SELECT `id_item`, `name`, `description`, `permanent`, `sort` FROM `groups_of_clients` WHERE `client_id` = '" . $chief['id'] . "' AND " . $chunk . "  ORDER by `sort` ASC LIMIT $start, $items_on_page");
                ...
                $items = $db->query("SELECT `id_item`, `name`, `description`, `permanent`, `sort` FROM `groups_of_clients` WHERE `client_id` = '" . $chief['id'] . "' ORDER by `sort` ASC LIMIT $start, $items_on_page");
                while ($group = $items->fetch_assoc()) {
                    $rows[] = array(
                        'id_item' => $group['id_item'],
                        'name' => protection($group['name'], 'display'),
...
if (isset($_GET['show']) and $_GET['show'] == 'true') {
    ...
    if ($count[0] > 0) {
        ...
        $groups_of_clients = $db->query("SELECT `id_item`, `name`, `description`, `permanent`, `sort` FROM `groups_of_clients` WHERE `client_id` = '" . $chief['id'] . "' ORDER by `sort` ASC LIMIT $start, $items_on_page");
        while ($group = $groups_of_clients->fetch_assoc()) {
            $rows[] = array(
                'id_item' => $group['id_item'],
                'name' => protection($group['name'], 'display'),
                'desc' => protection($group['description'], 'display'),
