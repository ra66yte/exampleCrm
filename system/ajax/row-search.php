...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($count[0] == 0) {
        ...
        foreach ($_POST as $key => $value) {
            if ($key != null && $value != null) {
                if ($key == 'id') {
                    ...
                    $search_products .= "SELECT DISTINCT `orders`.`id_item` FROM `orders` INNER JOIN `orders_products` ON (`orders_products`.`order_id` = `orders`.`id_item`) WHERE `orders_products`.`product_id` = '" . abs(intval($value)) . "' AND `orders_products`.`client_id` = '" . $chief['id'] . "' AND `orders`.`client_id` = '" . $chief['id'] . "'";
                    $search_keys[] = '';
                    $search_values[] = '';
                } else {
                    $search_keys[] = $key;
        ...
        if (count($search_values) != 0) {
            ...
            for ($i = 0; $i < count($search_values); $i++) {
                if ($search_keys[$i] != '') {
                    if ($search_keys[$i] == 'country') {
                        $chunk .= "AND `orders`.`" . protection($search_keys[$i], 'base') . "` = '" . protection($search_values[$i], 'base') . "'";
                    } else {
            ...
            if ($status == "all") {
                $count = $db->query("SELECT COUNT(*) FROM `orders` INNER JOIN `status_order` ON (`orders`.`status` = `status_order`.`id_item`) INNER JOIN `group_rights` ON (`group_rights`.`access_right` = '" . $right_id . "') WHERE `group_rights`.`group_id` = '" . $user['group_id'] . "' AND `group_rights`.`value` = `status_order`.`id_item` AND `group_rights`.`client_id` = '" . $chief['id'] . "' AND `status_order`.`status` = 'on' AND `status_order`.`client_id` = '" . $chief['id'] . "' AND `orders`.`deleted_at` = '0' AND `orders`.`client_id` = '" . $chief['id'] . "'" . ($chunk != '' ? " " . $chunk : '') . ($search_products != '' ? " AND `orders`.`id_item` IN (" . $search_products . ")" : '') . "")->fetch_row();
                ...
                if ($count[0] == 0) {
                    ...
                    $items = $db->query("SELECT `orders`.* FROM `orders` INNER JOIN `status_order` ON (`orders`.`status` = `status_order`.`id_item`) INNER JOIN `group_rights` ON (`group_rights`.`access_right` = '" . $right_id . "') WHERE `group_rights`.`group_id` = '" . $user['group_id'] . "' AND `group_rights`.`value` = `status_order`.`id_item` AND `group_rights`.`client_id` = '" . $chief['id'] . "' AND `status_order`.`status` = 'on' AND `status_order`.`client_id` = '" . $chief['id'] . "' AND `orders`.`deleted_at` = '0' AND `orders`.`client_id` = '" . $chief['id'] . "'" . ($chunk != '' ? " " . $chunk : '') . ($search_products != '' ? " AND `orders`.`id_item` IN (" . $search_products . ")" : '') . " ORDER by `orders`.`id` DESC LIMIT $start, $items_on_page");
                ...
                $count = $db->query("SELECT COUNT(*) FROM `orders` INNER JOIN `status_order` ON (`orders`.`status` = `status_order`.`id_item`) INNER JOIN `group_rights` ON (`group_rights`.`access_right` = '" . $right_id . "') WHERE `group_rights`.`group_id` = '" . $user['group_id'] . "' AND `group_rights`.`value` = `status_order`.`id_item` AND `group_rights`.`client_id` = '" . $chief['id'] . "' AND `status_order`.`status` = 'on' AND `status_order`.`client_id` = '" . $chief['id'] . "' AND `orders`.`deleted_at` = '0' AND `orders`.`status` = '" . $status . "' AND `orders`.`client_id` = '" . $chief['id'] . "'" . ($chunk != '' ? " " . $chunk : '') . ($search_products != '' ? " AND `orders`.`id_item` IN (" . $search_products . ")" : '') . "")->fetch_row();
                ...
                if ($count[0] == 0) {
                    ...
                    $items = $db->query("SELECT `orders`.* FROM `orders` INNER JOIN `status_order` ON (`orders`.`status` = `status_order`.`id_item`) INNER JOIN `group_rights` ON (`group_rights`.`access_right` = '" . $right_id . "') WHERE `group_rights`.`group_id` = '" . $user['group_id'] . "' AND `group_rights`.`value` = `status_order`.`id_item` AND `group_rights`.`client_id` = '" . $chief['id'] . "' AND `status_order`.`status` = 'on' AND `status_order`.`client_id` = '" . $chief['id'] . "' AND `orders`.`deleted_at` = '0' AND `orders`.`client_id` = '" . $chief['id'] . "' AND `orders`.`status` = '" . $status . "'" . ($chunk != '' ? " " . $chunk : '') . ($search_products != '' ? " AND `orders`.`id_item` IN (" . $search_products . ")" : '') . " ORDER by `orders`.`id` DESC LIMIT $start, $items_on_page");
