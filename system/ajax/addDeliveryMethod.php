...
if (isset($_GET['action']) and $_GET['action'] == 'submit' and $_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $delivery_name = isset($_POST['name']) ? protection($_POST['name'], 'base') : null;
    ...
    } elseif ($result = $db->query("SELECT COUNT(*) FROM `delivery_methods` WHERE `name` = '" . $name . "' AND `client_id` = '" . $chief['id'] . "'")->fetch_row() and $result[0] > 0) {
    ...
    if (!isset($error)) {
        ...
        if (move_uploaded_file($_FILES['ico']['tmp_name'], $path . $name)) {
            $success = 1;
            $count = $db->query("SELECT `delivery_methods` FROM `id_counters` WHERE `client_id` = '" . $chief['id'] . "'")->fetch_assoc();
            $id_item = $count['delivery_methods'] + 1;

            if ($result = $db->query("SELECT COUNT(*) FROM `delivery_methods` WHERE `id_item` = '" . $id_item . "' AND `client_id` = '" . $chief['id'] . "'")->fetch_row() and $result[0] == 0) {
                $db->query("INSERT INTO `delivery_methods` (`id`, `id_item`, `client_id`, `name`, `icon`) VALUES (null, '" . $id_item . "', '" . $chief['id'] . "', '" . $delivery_name . "', '" . protection($name, 'base') . "')");
            } else {
                $error = 'Не удалось добавить способ доставки!';
            }
        } else {
            $error = 'Не удалось загрузить иконку способа доставки!';
        }
    }
...
<script>
    $(function(){
        let form = $('#add-delivery-method'),
            btn = $('#button-add-delivery-method');

        form.find('#delivery-name').focus();

        $('#delivery-ico').on('change', function(e){
            let fileName = '';
            if (this.files) {
            ...
            // if ($(this).val() != '') $('#delivery-file').append('<button>X</button>');
        });
        ...
        function checkFields() {
            ...
            let uploadFile = form.find('#delivery-ico');

            if (uploadFile.val() == '') {
                error = 'Выберите иконку способа доставки!';
            } else {
            ...
            var name = form.find('#delivery-name').val().trim();
        ...
        $('#delivery-ico').on('change', function() {
            let error;
            let uploadFile = $(this);
            if (uploadFile.val() == '') {
                error = 'Выберите иконку способа доставки!';
        ...
        form.on('submit', function(e){
            ...
            if (error) {
                ...
                $.ajax({
                    type: "POST",
                    url: "/system/ajax/addDeliveryMethod.php?action=submit",
                    data: data,
                    processData: false,
                    contentType: false,
        ...
        <form id="add-delivery-method" method="post">
            ...
            <div class="modal-window-content__title">Конфигурация</div>
                <div class="modal-window-content__value">
                    <span>Название</span> <i class="fa fa-tag"> </i> <input id="delivery-name" type="text" name="name" placeholder="Введите название" autocomplete="off">
                </div>
                <div class="modal-window-content__value">
                    <span>Иконка</span> <i class="fa fa-picture-o"></i> <div id="delivery-file" class="modal-window-content__value-block" style="padding-left: 0">
                        <input id="delivery-ico" type="file" name="ico" class="inputfile"><label for="delivery-ico"><i class="fa fa-upload"></i> <b>Выберите файл</b></label>
                    </div>
                </div>
                <div class="buttons">
                    <button id="button-add-delivery-method" class="disabled" name="save-changes">Добавить</button>
