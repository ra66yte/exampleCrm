...
if (isset($_GET['module']) and $_GET['module'] == 'search') {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $count = $db->query("SELECT COUNT(*) FROM `payment_methods` WHERE `client_id` = '" . $chief['id'] . "'")->fetch_row();
        ...
        if ($count[0] == 0) {
            ...
            if (count($search_values) != 0) {
                ...
                if ($count[0] == 0) {
                    ...
                    $count = $db->query("SELECT COUNT(*) FROM `payment_methods` WHERE `client_id` = '" . $chief['id'] . "' AND " . $chunk . "")->fetch_row();
                    $countPages = k_page($count[0], $items_on_page);
                    $currentPage = page($countPages);
                    $start = ($currentPage * $items_on_page) - $items_on_page;

                    $items = $db->query("SELECT `id_item`, `icon`, `name`, `status`, `permanent`, `sort` FROM `payment_methods` WHERE `client_id` = '" . $chief['id'] . "' AND " . $chunk . "  ORDER by `sort` ASC LIMIT $start, $items_on_page");
                    while ($payment = $items->fetch_assoc()) {
                        $rows[] = [
                            'id_item' => $payment['id_item'],
                            'icon' => protection($payment['icon'], 'display'),
                            'name' => protection($payment['name'], 'display'),
                ...
                $items = $db->query("SELECT `id_item`, `icon`, `name`, `status`, `permanent`, `sort` FROM `payment_methods` WHERE `client_id` = '" . $chief['id'] . "' ORDER by `sort` ASC LIMIT $start, $items_on_page");
                while ($payment = $items->fetch_assoc()) {
                    $rows[] = [
                        'id_item' => $payment['id_item'],
                        'icon' => protection($payment['icon'], 'display'),
...
if (isset($_GET['show']) and $_GET['show'] == 'true') {
    ...
    if ($count[0] > 0) {
        ...
        $payments = $db->query("SELECT `id_item`, `icon`, `name`, `status`, `permanent`, `sort` FROM `payment_methods` WHERE `client_id` = '" . $chief['id'] . "' ORDER by `sort` ASC LIMIT $start, $items_on_page");
