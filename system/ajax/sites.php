...
if (isset($_GET['module']) and $_GET['module'] == 'search') {
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $count = $db->query("SELECT COUNT(*) FROM `sites` WHERE `client_id` = '" . $chief['id'] . "'")->fetch_row();
        if ($count[0] == 0) {
            ...
            if (count($search_values) != 0) {
                ...
                $count = $db->query("SELECT COUNT(*) FROM `sites` WHERE `client_id` = '" . $chief['id'] . "' AND " . $chunk . "")->fetch_row();
                ...
                if ($count[0] == 0) {
                    ...
                    $items = $db->query("SELECT `id_item`, `name`, `url`, `sort` FROM `sites` WHERE `client_id` = '" . $chief['id'] . "' AND " . $chunk . " ORDER by `id` LIMIT $start, $items_on_page");
                    while ($site = $items->fetch_assoc()) {
                        $rows[] = array(
                            'id_item' => $site['id_item'],
                            'name' => protection($site['name'], 'display'),
                            'url' => protection($site['url'], 'display'),
                            'sort' => $site['sort']
                        );
                    }
                }
                    ...
                    $items = $db->query("SELECT `id_item`, `name`, `url`, `sort` FROM `sites` WHERE `client_id` = '" . $chief['id'] . "' ORDER by `id` LIMIT $start, $items_on_page");
                    while ($site = $items->fetch_assoc()) {
                        $rows[] = array(
                            'id_item' => $site['id_item'],
                            'name' => protection($site['name'], 'display'),
...
if (isset($_GET['show']) and $_GET['show'] == 'true') {
    ...
    if ($count[0] > 0) {
        ...
        $currentPage = page($countPages);
        $start = ($currentPage * $items_on_page) - $items_on_page;
        $sites = $db->query("SELECT `id_item`, `name`, `url`, `sort` FROM `sites` WHERE `client_id` = '" . $chief['id'] . "' ORDER by `id` LIMIT $start, $items_on_page");
