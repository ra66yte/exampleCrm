        ...
        initEvent: function () {
            ...
            scope.on('click.mSelect', options.selector, function (e) {
                ...
                */
                    ...
                    } else if (!$(this).hasClass('disabled') && !$(this).hasClass('blocked-row')) {
                ...
                } else if (!e.ctrlKey && !e.metaKey && !deferred_click && !self.mobile_touch) { // self.mobile.touch на мобиле несколь выделений подряд
                    $(this).siblings().removeClass(actcls);
                }
                self.last = $(this);

                if (!$('tr.' + actcls).length) $('button#button-select-all').removeAttr('style');
            ...
            $(document).on('click.mSelect', function (e) {
                ...
                scope.find(options.selector).each(function () {
                    if (!self.checkStatics($(this))) {
                        $(this).removeClass(actcls); 
                    } else if (!$('.modal-window-wrapper').length && $('.status-panel__options').is(':hidden')) { // Если нет модальных окон и поп'апа с опциями
                        

                      $(this).removeClass('static'); // Убираем статик, решает проблему с кликом по таб статусу после дабл клика
            ...
            $(document).on('keydown.mSelect', function (e) {
                ...
                if ((e.keyCode == 65) && (e.metaKey || e.ctrlKey)) {
                    ...
                    $(options.selector, scope).each(function () {
                        if (!self.checkStatics($(this)) && $('.status-panel__options').is(':hidden') && !$('.substrate').length && !$(this).hasClass('disabled') && !$(this).hasClass('blocked-row') && $(this).css('display') != 'none') { // Блокируем выделение, если открыто меню опций над заказами
                            $(this).addClass(actcls);
                        }
                    });
