...
if (!checkAccess('payment_methods')) redirect('/denied');

$data['title'] = 'Способы оплаты';
...
<script>
    ...
    $(function(){
        // Поиск по таблице
        let form = $('#form-payment-methods'),
            table = $('#payment-methods__table');
        ...
        form.on('submit', function(e){
            ...
            $.ajax({
                type: "POST",
                url: "/system/ajax/payment_methods.php?module=search&page=" + page,
                data: data,
                beforeSend: function() {
                    startPreloader();
                },
                success: function(response) {
                    ...
                    $('#payment-methods__table tbody').html('');
                    // Items
                    tableRows = renderRows(jsonData.rows, true);
                    $('#payment-methods__table tbody').html(tableRows);
        ...
        $('#form-payment-methods').on('dblclick', 'tr.table__item', function(e) {
            if (!$(this).hasClass('disabled')) {
                ...
                $.ajax({
                    type: "POST",
                    url: "/ajax_viewPaymentMethod?payment_id=" + payment_id + "&query=get_title",
                    data: {},
                    success: function(response) {    
                        let jsonData = JSON.parse(response);
                        if (jsonData.success == 1) {
                            showModalWindow('Способ оплаты #' + payment_id + ': ' + jsonData.title.payment_name, '/ajax_viewPaymentMethod?payment_id=' + payment_id);
                        }
                    }    
                });
            }
        ...
        $('.status-panel').on('click', '#button-edit', function(e) {
            if (!$(this).hasClass('disabled')) {
                let payment_id = $('tr.table__active').attr('data-id');
                $.ajax({
                    type: "POST",
                    url: "/ajax_viewPaymentMethod?payment_id=" + payment_id + "&query=get_title",
                    data: {},
    ...
    function RemoveSearchField(field) {
        $.each($('tr input'), function(e) {
            if ($(this).attr('name') == field) {
                ...
                $(this).val('');
                $('#form-payment-methods').trigger('submit');
            }
        });
    ...
    function renderRows(data, search = false) {
        let tableRows = '',
            countTd = $('#payment-methods__table thead').find('tr td').length;

        if (Array.isArray(data) && data.length) {
            $.each(data, function(i, item){
                tableRows += ('<tr data-id="' + item.id_item + '" class="table__item' + (item.permanent == 'on' ? ' disabled' : '') + '">' +
                                '<td>' + item.id_item + '</td>' +
                                '<td class="center table__item-icon"><img src="/system/images/payment/' + (item.permanent == 'on' ? '' : item.chief_id + '/') + item.icon + '" alt="pay"></td>' +
                                '<td>' + item.name + '</td>' +
                                '<td align="center">' +
                                    '<label class="toggle">' +
                                        '<input' + (item.permanent == 'on' ? ' disabled' : '') + ' type="checkbox"' + (item.status == 'on' ? ' checked' : '') + ' onclick="changeStatus(\'payment_methods\', \'' + item.status + '\', \'' + item.id_item + '\');" class="toggle__input">' +
    ...
    function loadPaymentMethods(page) {
        if (!page) {
            page = getParameterByName('page') ? getParameterByName('page') : 1;
        } else {
            let stateObj = { foo: "payment_methods" };
            history.pushState(stateObj, "payment_methods", '?page=' + page);
        }
        $('.status-panel__count').text('').hide();
    
        $.ajax({
            type: "POST",
            url: "/system/ajax/payment_methods.php?show=true&page=" + page,
            data: {},
            beforeSend: function() {
                startPreloader();
            },
    ...
    function addPaymentMethod() {
        showModalWindow('Добавление нового способа оплаты', '/system/ajax/addPaymentMethod.php');
    }

    function deletePaymentMethods() {
        let countPayments = $('tr.table__active').length;
        if (countPayments < 1) {
            console.log('Ничего не выбрано..');
        } else {
            let name = 'способа оплаты';
            if (countPayments > 1) name = 'способов оплаты';
            showModalWindow('Удаление ' + name, '/ajax_deletePayment?count=' + countPayments, 'confirm');
        }
    }
</script>
            ...
            <section class="content">
                    ...
                    <div style="position: relative">
                        ...
                        <div class="status-panel__options">
                            <ul>
                                ...
                                <li class="hr"></li>
                                <li id="button-delete" class="disabled"><i class="fa fa-trash-o"></i> Удалить</li> <!-- onclick="deletePaymentMethods();" style="color: #AE0000" -->
                            </ul>
                        </div>
                    </div>
                    <button id="button-add" style="color: green" onclick="addPaymentMethod();" disabled><i class="fa fa-plus-square"></i> Добавить</button>
                ...
                <div class="content__overflow">
                    <form id="form-payment-methods" method="post" autocomplete="off">
                        <table id="payment-methods__table" class="table" cellpadding="0" cellspacing="0">
                            <thead>
                                <tr>
                                ...
                                <tr class="table-row-search">
                                    <td style="max-width: 20px"><input type="text" name="id" spellcheck="false"></td>
                                    <td></td>
