...
if (!checkAccess('currencies')) redirect('/denied');
...
<script>
    $(function() {
        // Поиск по таблице
        let form = $('#form-сurrencies'),
            table = $('#сurrencies__table');
        ...
        form.on('submit', function(e){
            ...
            $.ajax({
                type: "POST",
                url: "/system/ajax/currencies.php?module=search&page=" + page,
                data: data,
                beforeSend: function() {
                    startPreloader();
                },
                success: function(response) {
                    ...
                    $('#сurrencies__table tbody').html('');
                    // Items
                    tableRows = renderRows(jsonData.rows, true);

                    $('#сurrencies__table tbody').html(tableRows);;
        ...
        $('#form-сurrencies').on('dblclick', 'tr.table__item', function(e) {
            if (!$(this).hasClass('disabled')) {
                ...
                $.ajax({
                    type: "POST",
                    url: "/system/ajax/viewCurrency.php?currency_id=" + currency_id + "&query=get_title",
                    data: {},
                    success: function(response) {    
                        let jsonData = JSON.parse(response);
                        showModalWindow('Валюта - <b>' + jsonData.title.currency_name + '</b>', '/system/ajax/viewCurrency.php?currency_id=' + currency_id);
                    }    
                });
            }
        });
        ...
        $('.status-panel').on('click', '#button-edit', function(e) {
            if (!$(this).hasClass('disabled')) {
                let currency_id = $('tr.table__active').attr('data-id');
                $.ajax({
                    type: "POST",
                    url: "/system/ajax/viewCurrency.php?currency_id=" + currency_id + "&query=get_title",
                    data: {},
    ...
    function RemoveSearchField(field) {
            $.each($('tr input'), function(e) {
                if ($(this).attr('name') == field) {
                    ...
                    $(this).val('');
                    $('#form-сurrencies').trigger('submit');
                }
            });
    ...
    function renderRows(data, search = false) {
        let tableRows = '',
            countTd = $('#сurrencies__table thead').find('tr td').length;
        ...
        if (Array.isArray(data) && data.length) {
            $.each(data, function(i, item){
                tableRows += ('<tr data-id="' + item.id_item + '" class="table__item">' +
                                '<td>' + item.id_item + '</td>' +
                                '<td>' + item.name + '</td>' +
                                '<td class="center">' + item.symbol + '</td>' +
    ...
    function loadCurrencies(page) {
        if (!page) {
            ...
            let stateObj = { foo: "currencies" };
            history.pushState(stateObj, "currencies", '?page=' + page);
        }
        $('.status-panel__count').text('').hide();
    
        $.ajax({
            type: "POST",
            url: "/system/ajax/currencies.php?show=true&page=" + page,
            data: {},
            beforeSend: function() {
                startPreloader();
            },
    ...
    function addCurrency() {
        showModalWindow('Добавление валюты', '/system/ajax/addCurrency.php');
    }

    function deleteCurrencies() {
        let countCurrencies = $('tr.table__active').length;
        if (countCurrencies < 1) {
            console.log('Ничего не выбрано..');
        } else {
            let name = 'валюты';
            if (countCurrencies > 1) name = 'валют';
            showModalWindow('Удаление ' + name, '/system/ajax/deleteCurrencies.php?count=' + countCurrencies, 'confirm');
        }
    }
</script>
                <!-- Content -->
                    ...
                    <div style="position: relative">
                        ...
                        <div class="status-panel__options">
                            <ul>
                                ...
                                <li class="hr"></li>
                                <li id="button-delete" onclick="deleteCurrencies();" style="color: #AE0000"><i class="fa fa-trash-o"></i> Удалить</li>
                            </ul>
                        </div>
                    </div>
                    <button id="button-add" style="color: green" onclick="addCurrency();"><i class="fa fa-plus-square"></i> Добавить</button>
                ...
                <div class="content__overflow">
                    <form id="form-сurrencies" method="post" autocomplete="off">
                        <table id="сurrencies__table" class="table" cellpadding="0" cellspacing="0">
                            <thead>
                                <tr>
                                    <th data-name="id" style="min-width: 30px">id</th>
                                    <th data-name="name">Название</th>
                                    <th data-name="symbol">Символ</th>
                                ...
                                <tr class="table-row-search">
                                    ...
                                    <td><input type="text" name="symbol" style="max-width: 80px"></td>
                                    <td></td>
                                </tr>
                            </thead> 
                            <tbody>
